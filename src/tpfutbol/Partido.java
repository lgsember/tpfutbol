...
public class Partido {
	...
	public Equipo JugarPartido() {
		...
		if (this.getEquipo1().getJugadores().size() < 11) {
			JOptionPane.showMessageDialog(null,
					equipo1.getNombre() + " no tiene jugadores suficientes para jugar el partido.");
			return null;
		} else if (this.getEquipo2().getJugadores().size() < 11) {
			JOptionPane.showMessageDialog(null,
					equipo2.getNombre() + " no tiene jugadores suficientes para jugar el partido.");
			return null;
		} else {
			JOptionPane.showMessageDialog(null, "Empieza el partido entre " + this.getEquipo1().getNombre() + " y "
			...
			for (int i = 0; i < 6; i++) {
				...
				switch (lance) {
				case 0:
					JOptionPane.showMessageDialog(null, this.getEquipo1().getNombre() + " se acerca a la portería de "
							+ this.getEquipo2().getNombre());
					ponte = (int) (Math.random() * 2);
					if (ponte == 0) {
				...
				case 2:
					...
					if (this.getEquipo1().getJugadores().get(jugador).isTitularidad()) {
						if (this.getEquipo1().getJugadores().get(jugador).isMolestado()) {
							JOptionPane.showMessageDialog(null,
									"Posicion adelantada de "
											+ this.getEquipo1().getJugadores().get(jugador).getNombre() + " del equipo "
											+ this.getEquipo1().getNombre());
						} else {
							JOptionPane.showMessageDialog(null,
									"El jugador " + this.getEquipo1().getJugadores().get(jugador).getNombre()
											+ " del equipo " + this.getEquipo1().getNombre() + " se lesionó.");
							this.getEquipo1().getJugadores().get(jugador).setMolestado(true);
						}
					}
					break;
				case 3:
					JOptionPane.showMessageDialog(null, this.getEquipo2().getNombre() + " se acerca a la portería de "
							+ this.getEquipo1().getNombre());
					ponte = (int) (Math.random() * 2);
					if (ponte == 0) {
						JOptionPane.showMessageDialog(null, "Patada al arco");
				...
				case 5:
					...
					if (this.getEquipo2().getJugadores().get(jugador).isTitularidad()) {
						if (this.getEquipo2().getJugadores().get(jugador).isMolestado()) {
							JOptionPane.showMessageDialog(null,
									"Posicion adelantada de " + this.getEquipo2().getNombre());
						} else {
							JOptionPane.showMessageDialog(null,
									"El jugador " + this.getEquipo2().getJugadores().get(jugador).getNombre()
											+ " del equipo " + this.getEquipo2().getNombre() + " se lesionó.");
			...
			if (this.getGoles1() == this.getGoles2()) {
				...
				JOptionPane.showMessageDialog(null, "El equipo que ganó es " + getEquipo1().getNombre()
						+ "\nResultado: " + this.getGoles1() + " x " + this.getGoles2());
				return this.getEquipo1();
			} else {
				JOptionPane.showMessageDialog(null,
						"El equipo que ganó es " + "\nResultado: " + this.getGoles2() + " x " + this.getGoles1());
				return this.getEquipo2();
			}
